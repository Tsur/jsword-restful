...
public class Main
{
	...
	public static List<Book> getAllBibles()
    {
	...
	public static Book getBible(String bookInitials)
    {
		return Books.installed().getBook(bookInitials);
    }
	
	public static List<Book> getAllDictionaries()
    {
     ...
     */
    public String readStyledHTML(String bookInitials, String reference, int maxKeyCount) throws NoSuchKeyException, BookException, TransformerException, SAXException
    {
        Book book = getBook(bookInitials);
        ...
        //System.out.println(osissep.toString());

        Converter styler = new ConverterHTML();
    ...
    public String readStyledText(String bookInitials, String reference, int maxKeyCount) throws NoSuchKeyException, BookException, TransformerException, SAXException
    {
        ...
        Converter styler = new ConverterTEXT();
    ...
    public void search() throws BookException
    {
        ...
        System.out.println("Obtenemos biblia donde buscar"); //$NON-NLS-1$
    ...
    public String search(String book, String search) throws BookException
    {
        Book bible = Books.installed().getBook(book);
        //System.out.println("Obtenemos biblia donde buscar"); //$NON-NLS-1$

        // This does a standard operator search. See the search documentation
        // for more examples of how to search
        Key key = bible.find(search); //$NON-NLS-1$
    ...
    public void installBook()
    {
        ...
        System.out.println("Creamos un instalardor y le preguntamos que sitios sabe"); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        System.out.println("obtenemos lista de libros disponibles "); 
        
        for(Object b : availableBooks)
        {
        	Book book = (Book) b;
        	
        	System.out.println("Nombre es "+book.getName()); 
        }
        
        System.out.println("obtenemos un nombre en concreto");
        
        // get some available books. In this case, just one book.
        availableBooks = installer.getBooks(new MyBookFilter("KJV")); //$NON-NLS-1$
        ...
        if (book != null)
        {
            ...
            try
            {
                if (Books.installed().getBook("KJV") != null) //$NON-NLS-1$
                {
                	System.out.println("desinstalamos"); //$NON-NLS-1$ //$NON-NLS-2$
            ...
            try
            {
            	System.out.println("ahora lo instalamos"); 
     ...
     */
    public static void main(String[] args) throws BookException, NoSuchKeyException, TransformerException, SAXException, URISyntaxException
    {
        Main examples = new Main();

        //LuceneIndex li = new LuceneIndex(examples.getBible("SpaRV"),new URI("file:///home/zuri/.jsword/lucene/Sword/SpaRV/"),true);
        
        //examples.installBook();
        
        //System.out.println("The plain text of Gen 1:1 is " + examples.getPlainText("KJV", "Mat 1:1")); //$NON-NLS-1$ //$NON-NLS-2$
        //System.out.println("The plain text of Gen 1:1 is " + examples.getPlainText("bwe", "Mat 1:1")); //$NON-NLS-1$ //$NON-NLS-2$
        //System.out.println("The plain text : " + examples.getPlainText("chincvs", "ex 1:1")); //$NON-NLS-1$ //$NON-NLS-2$
        
        //System.out.println("The html text of Mat 1 is " + examples.readStyledText("bwe", "Mat 1", 100)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		BibleList list = new BibleList();
		
		for(Book b: Main.getAllBibles())
		{
			Bible bible = new Bible(b.getInitials());
			
			bible.setName(b.getName());
			
			bible.setLanguage(b.getLanguage());
