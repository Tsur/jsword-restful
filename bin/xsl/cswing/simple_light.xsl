 ...
 *
 * ID: $Id: simple.xsl 1874 2008-06-10 21:40:19Z dmsmith $
 -->
 <!--
 ...
 <xsl:stylesheet
  ...
  <xsl:template match="verse[not(@eID)]">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <p><xsl:call-template name="versenum"/><xsl:apply-templates/></p>
  ...
  <xsl:template match="verse[not(@eID)]" mode="jesus">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <p><xsl:call-template name="versenum"/><xsl:apply-templates mode="jesus"/></p>
        <!-- Follow the verse with an extra space -->
        <!-- when they don't start on lines to themselves -->
        <xsl:text> </xsl:text>
      </xsl:otherwise>
